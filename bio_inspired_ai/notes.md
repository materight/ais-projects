...
- **What kind of performance do you get, in general, on the tested problems? What happens when you change the parametrization of GP?**
    ...
    - *ant:* Tithe the default parameters, the best individual reached a fitness of 67 over 89, with a tree of 7 levels. By changing the parameters, in particular the crossover probability to 0.8 and the HOF size to 3, the GP was able to reach a best individual fitness of 74. However, the resulting tree is very complex, with 16 levels and many nodes. In this case a worst but simpler solution may be preferable, to avoid losing the interpretability of the final controller.
...
The algorithm was tested using as generator function `sin(x)+5*x^2`, that the standard GP was not able to compute correctly, as seen in Exercise 1. This version of "competitive GP" instead was able to reconstruct the target function correctly (with the default parameters). In particular, the final best tree was `((x*x)+((((x+x)*1)+((x+x)*1))*x))+sin(x)` that is equal to `sin(x)+5*x^2`, and therefore obtained a fitness of 0. The interesting aspect is that the algorithm was able to converge to the optima solution after just 20 generations.
...
A more significant impact on the GP performance is given by changing the `GA_REP_IND` parameter. In particular, a lower value (e.g. 3), still allow the GP to converge to the optimal solution, but the resulting tree is very complex (more than 20 levels). This is probably because the GP algorithm is pushed to overfitting the samples with a low number of samples in each GA individual.
