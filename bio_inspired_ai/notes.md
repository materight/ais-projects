...
With a single perceptron, the model was able to solve the "Or" problem with a final best fitness of ~0.0009. The resulting network has a weight for both input connection of +8.0, while the bias is -3.85. This is because since we are using a sigmoid activation that limits the output to [0, 1], the network tries to maximize the output when a 1 is given in input, and therefore the two weights connected to the input nodes have the maximum possible weight. A negative bias instead is mainly useful when the input is [0, 0]: in this case the input nodes multiplied by their weights will result in 0, and since `sigmoid(0)=0.5`, the net tries to shift the result towards a negative value to obtain a final result close to zero. \
...
No, the best fitness obtained is ~1.0. This is because a single perceptron is only able to solve linearly separable problems, and the "Xor" is not one of them. Only by adding an additional layer we can add non-linearity and be able to solve it.
...
### Exercise 3 (NEAT)
- **First, run a single instance of each of the two configurations (with/without elitism). What do you observe? Is the algorithm without elitism able to converge to the optimal fitness value? What about the algorithm with elitism? What is the effect of elitism on convergence? What about the number of species and their dynamics?** \
In both cases the algorithm was able to converge to an optimal fitness, in case of "no-elitism" to ~3.95 and in case of "elitism" to ~3.96. However, while with elitism the algorithm converged at generation 37, the version without elitism took 73 generations, and if tested on multiple run was not always able to converge to a fitness greater than 3.9.
...
Yes, the results over 10 runs show that the algorithm with elitism is a better choice. In particular, the average fitness with elitism is ~3.9, while without is ~3.4. Moreover, the deviation of the fitnesses obtained with elitism is much lower, showing that it obtained better results overall.

### Questions
- **What is the genotype and what is the phenotype in the problems considered in this lab?** \
In the first two exercise, the genotype would be the vector representing each weight and bias values. In the last exercise (NEAT), the genotype encodes the weights but also the topology of the network, in particular its connections between nodes. In all the problems, the phenotype is the network instance, created according to the predefined or encoded topology and weights assigned to each synapse.
...
Hidden nodes increase the representational power of a neural network. Each perceptron learns an hyperplane that separates the input space in two parts. Therefore, if our problem is linearly separable, a single perceptron is enough to solve it (e.g. with "And" or "Or"). However, with more complex problem a single perceptron may be not enough, since a single hyperplane is unable to divide solutions in a non-linear problem. By implementing a Multi-Layer Perceptron these issue can be solved as well, since each layer will learn a new input representation s.t. the last layer receive as input a linearly-separable input space.
...
When a solution to a problem depends on the previous inputs and solutions, a recurrent network is needed to implement a sort of "memory" of the last input. If we use a simple feedforward NN, this kind of memory is not present and the network may be unable to solve the problem, even with multiple layers. As explained before, multiple hidden units are required in the case of non-linearly separable problems.
...
Yes, the EA is able to evolve a NN that can reach the target. With the custom fitness described above, the robot goes quite directly for the target, with just an initial turn to the right. The most peculiar characteristic is that it goes backward, but since it doesn't change anything in terms of velocity, this is not a problem and therefore it is not considered in the fitness computation.
...
The best candidate evolved previously (the one that receives in input also the IR sensor data) was not able to reach the target in the new problem with obstacles. Since the robot learned to go directly towards the target, it tries to go downward but continues to bump on the wall and ultimately get stuck in the dead-end corridor. Therefore it is not able to generalize to more complex environments.
...
Using the previous fitness function (`distanceToTarget/(pathLength/timestepToReachTarget)`), the best individual obtained is able to reach the target, but it takes a lot of time even if it moves quickly because the exploration is very chaotic (it moves in circles) and the robot end up trying the same path multiple times. By changing completely the approach, a very good solution (only ~38 seconds to reach the target) was obtained by using as fitness the fraction of time spent to reach the target w.r.t. the total individual lifetime (i.e. `fitness = timestepToReachTarget/self.nrTimeStepsGen`), to try to minimize it and spend as much time as possible on the target. The final best individual obtained with its path can be seen in the image below.
...
By changing the starting position to (400, 800) and the target position to (3500, 3200), it is clear that the best evolved robot is not able to generalize correctly to new situations, since it is not able to reach the target and instead keeps moving on the same paths again and again. Instead, the best individual, obtained using as fitness `distanceToTarget/(pathLength/timestepToReachTarget)`, is able to reach the target in ~130 seconds, even with this new configuration. This is probably because it follows a circular moving pattern, favoring the exploration of the environment.
...
By increasing `P_NUM_SEQ` instead, we are increasing the number of sequences each parasite shuffles, i.e. we are increasing the number of individuals tested and therefore the maximum fitness. From a performance point of view, the results do not changes significantly, since increasing `P_NUM_SEQ` have a similar effect of increasing the number of generations. For this reason however, the resulting hosts have lower fitnesses since they are evolved over more sequences. \
...
The first example that comes to mind is similar to what seen during the lecture, i.e. the competitive co-evolution of an agent and an environment. This could be used for example for training robots that are going to operate in an high-risk environment, e.g. to provide first-aid during a fire or after an earthquake. By applying the competitive co-evolution paradigm, we are able to initially train a robot to navigate and perform simple tasks in a simple environment, without obstacles or unexpected events. Then, by progressively increasing the complexity of the environment, we may be able to obtain agents that are capable of performing under very complex and risky conditions.

- **Can you think of some other competitive co-evolutionary dynamics in nature different from the prey-predator case?** \
An example may be the plant-vs-herbivore case, where plants evolve to defend from herbivores, e.g. developing fruits with harder shells or spikes.
...
- **What kind of performance do you get, in general, on the tested problems? What happens when you change the parametrization of GP?**
    - *parity:* the GP algorithm is able to correctly reach the optimal individual with a fitness of 64 (the maximum possible value) without any particular parameter tuning. However, if we just increase the problem size to 8 bit instead of 6 (therefore the max fitness becomes 2^8=256), the best fitness obtained by the GP is only 160. By trying different parameters to increase the diversity of the solutions, in particular changing the crossover rate to 0.8, the tournament size to 7 and the "Hall of Fame" size to 3, the GP was able to reach a best fitness of 192. Additional modification to the parameters didn't provide any significant benefit.
    - *multiplexer:* in this case the best individual is able to reach a fitness of only 1472 (tested again over multiple runs), while the possible maximum fitness is 2048 and the resulting tree is very complex, with many nodes. This is an issue particularly during the evaluation, since the process becomes slower and slower as the number of generations increases, since the trees become more and more complex. Trying to increase the number of generations didn't provide any additional benefit, also because DEAP is not able to handle trees with a depth larger than 90 levels. Increasing the population size to 100 allowed the algorithm to reach a best fitness of 1496, slightly better than the previous one. A significant performance boost was obtained by increasing the probability of crossover and mutation, from respectively 0.8 and 0.1 to 0.9 and 0.2, reaching a fitness of 1600. The last successful modification was to increase the "Hall of Fame" size from 1 to 3, obtaining a score of 1664.
    - *spambase:* After some tests, the default parameters were able to reach a maximum fitness of 364, with a rather simple tree comprised of just 10 nodes. By changing the parameters and trying different configurations over multiple seeds, the best results were obtained using a crossover and mutation rate of 0.9 and 0.3, a tournament size of 7 and a "Hall of Fame" size of 3, for a final best fitness of 373, an accuracy of ~93%.
    - *ant:* With the default parameters, the best individual reached a fitness of 67 over 89, with a tree of 7 levels. By changing the parameters, in particular the crossover probability to 0.8 and the HOF size to 3, the GP was able to reach a best individual fitness of 74. However, the resulting tree is very complex, with 16 levels and many nodes. In this case a worse but simpler solution may be preferable, to avoid losing the interpretability of the final controller.
...
The algorithm was tested using as generator function `sin(x)+5*x^2`, that the standard GP was not able to compute correctly, as seen in Exercise 1. This version of "competitive GP" instead was able to reconstruct the target function correctly (with the default parameters). In particular, the final best tree was `((x*x)+((((x+x)*1)+((x+x)*1))*x))+sin(x)` that is equal to `sin(x)+5*x^2`, and therefore obtained a fitness of 0. The interesting aspect is that the algorithm was able to converge to the optimal solution after just 20 generations.
...
A larger population provides overall a lower average fitness w.r.t to lower sizes. Viceversa for a smaller population. However, in the end, the difference in the results of the default population size vs large population is not significant, since both reach the optimum solution requiring on average the same number of generations. \
A more significant impact on the GP performance is given by changing the `GA_REP_IND` parameter. In particular, a lower value (e.g. 3), still allows the GP to converge to the optimal solution, but the resulting tree is very complex (more than 20 levels). This is probably because the GP algorithm is pushed to overfit the samples, having a low number of samples in each GA individual.
