%Zig-zag coding
% note: [Out,M,N]=codezz_a(IN);
% 
%   where "IN"  is the input matrix
%   dove "Out" is the row matrix with coefficients

function [Out]=codezz(IN)

%zig zag coding
zz=blkproc(IN,[8 8],'zigzag(x)');
sM=size(IN);
M=sM(1);
N=sM(2);

%all coefficient in a run
Out=[];
for i=1:size(zz,1)
    Out=cat(2,Out,zz(i,:));
end
